...
export default async function handler(req) {
  ...
  const segments = url.pathname.split("/").filter(Boolean); // e.g. ["api","genesys","save"]
  ...
  if (!apiKey || apiKey !== process.env.API_KEY) {
    // Allow base /api/genesys to show help without auth
    if (op !== "genesys") return bad("unauthorized", 401);
  }
