...
// --- Index helpers (now stores { id, name }) ---
...
async function upsertIndex(id, name){
  ...
  if (i >= 0) {
    ...
    if (typeof name === "string" && name && items[i].name !== name) {
      items[i].name = name;
...
export default async function handler(req) {
  ...
  const op = url.pathname.split("/").pop(); // last path segment
  ...
  // --- LIST (now returns array of { id, name }) ---
  ...
  return ok({ ok:true, endpoints:[
    "POST /api/genesys/save",
    "GET  /api/genesys/get?playerId=...",
    "GET  /api/genesys/list",
    "POST /api/genesys/delete",
    "POST /api/genesys/roll"
  ]});
}
